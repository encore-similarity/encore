...
# Copyright (C) 2014 Matteo Tiberti, Wouter Boomsma
#
...
"""
Calculate entropy --- :mod:`encore.entropy`
=====================================================================
...
def calc_entropy(ensemble,
    ...
    **Arguments**:
    ...
    `estimator` : covariance estimator object (MLEstimator or ShrinkageEstimator)
        which estimator type to use (maximum likelihood, shrinkage)

    `align` : bool
        whether to align the structures first to the reference conformation
    ...
    `start` : int
        Start frame index, relative to the ensemble

    `end` : int
        End frame index, relative to the ensemble

    `temperature` : float
        Temperature (Kelvin)

    `omit_last_eigenvalues` : int
        How many of the smallest eigen values to omit (these are close to zero when aligning the ensemble and can be omitted)

    **Returns**:

    `entropy` : float
        Entropy value ( J/(K*mol) )
    '''
...
def calc_entropy_progression(ensemble,
    ...
    '''
    Calculate, for 0-t, with increasing t - the entropy from a covariance matrix, using the technique described in
    ...
    **Arguments**:
    ...
    `realign` : bool
        Whether to realign in each step

    `steps` : int
        How many steps to make in the progression

    **Returns**:

    `entropies` : numpy.array
        Entropy values ( J/(K*mol) )
    '''
